<?php
	class EnrollmentChange extends page
	{
		function post()
		{
			//include '../www/Logins/CollegeDataLogin.php';
			include 'DataManip.php';
			$this->title = 'Highest Enrollemnt Percentage Change';
			include 'DBLogin.php';
			$CollegeData = new DBManip($host, $dbname, $user, $pass);
			$this->content.='<div class="container">';
			$this->content.= '
			<div class="jumbotron">
				<h1 class="CollegeText" style="color: #FC91FF;">Highest Enrollment Percentage Change</h1>
			</div>
			'; 
			$this->content.= '<h1>College Data of Highest Enrllment Percentage Change</h1>
			...
			;
			$this->content.=$CollegeData->queryPercentChangeEnrollment($_REQUEST['Range'], 'BarChart').'<hr>';	
			$this->content.= '<h3 id="List"3> List </h3>';			
			$this->content.=$CollegeData->queryPercentChangeEnrollment($_REQUEST['Range'], 'Table').'<br>';
