<?php
	class LNRPerStudent extends page
	{
		function post()
		{
			...
			$this->content.= '
			<div class="jumbotron">
				<h1 class="CollegeText" style="color: #D60303;">'.$_REQUEST['LNR'].' Per Student for the year of '.$_REQUEST['Year'].'</h1>
			</div>
			'; 
			$this->content.= '<h1>College Data of '.$_REQUEST['LNR'].' Per Student for the year '.$_REQUEST['Year'].'</h1>
			...
			;
			$this->content.=$CollegeData->queryLargestLNRPerStudent($_REQUEST['Year'], $_REQUEST['LNR'], $_REQUEST['Range'], 'BarChart').'<hr>';	
			$this->content.= '<h3 id="List"3> List </h3>';			
			$this->content.=$CollegeData->queryLargestLNRPerStudent($_REQUEST['Year'], $_REQUEST['LNR'], $_REQUEST['Range'], 'Table').'<br>';
